    ...
    <>
      ...
      <div className="upload-history">
        ...
        <table>
          ...
          <tbody>
            {isLoading ? (
              <tr><td colSpan={5} style={{ textAlign: 'center', padding: 24, color: 'var(--color-text-muted)' }}>جاري التحميل…</td></tr>
            ) : !history?.length ? (
              <tr><td colSpan={5} style={{ textAlign: 'center', padding: 24, color: 'var(--color-text-muted)' }}>لا يوجد سجل رفع بعد</td></tr>
            ) : history.map((h) => (
              <tr key={h.id}>
                <td style={{ fontFamily: 'var(--font-en)', fontSize: 12 }}>{h.file_name}</td>
                <td style={{ fontFamily: 'var(--font-en)', fontSize: 12 }}>
